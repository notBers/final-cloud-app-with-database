...
def show_exam_result(request, course_id, submission_id):
    ...
    for question in course.question_set.all():
        question_correct_choices = question.choice_set.filter(is_correct=True)
        selected_choices = submission.choices.filter(question=question)
        if set(question_correct_choices) == set(selected_choices):
            correct_choices += 1

    score = (correct_choices / total_questions) * 100
    context = {
        'course': course,
